...
SIMPLE_STATS="restore_time_replay restore_time_read
    restore_time_write restore_time_openscan backup_not_prefetched"
...
for d in restore-*; do
    ...
    restorevol=$(grep restore_log_volume $d/out1.txt | tail -n 1 | awk '{print $2}')
    logvol=$(grep la_read_volume $d/out1.txt | awk '{print $2}')
    # lareads=$(($LA_BLOCK_SIZE * 1024 * $(grep la_block_reads $d/out1.txt | tail -n 1 | awk '{print $2}')))
    echo $TITLE $(bc -l <<< "$restorevol / $logvol") >> la_read_efficiency.txt
    ...
    gnuplot -e "outfile='mpstat_"$TITLE".png'; file='mpstat_"$TITLE".txt'; segsize="$SEGSIZE"" mpstat.gp
done
